class AiService
  ...
  def process_query(user_query, profile:, context: nil)
...

    {
      ...
      response_confidence: response_data&.dig(:confidence)
    }
  rescue BenefitsCoverageAssistant::AiServiceError => e
    Rails.logger.error("AI Service error: #{e.message}")
    build_error_response("I'm having trouble processing your request right now. A support ticket will be created.")
  end
